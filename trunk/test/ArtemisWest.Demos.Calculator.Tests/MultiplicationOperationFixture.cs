...
{
    [TestClass]
    public sealed class MultiplicationOperationFixture
    {
        [TestMethod]
        public void Should_set_value_to_sum_of_base_operator_values()
        {
            ...
            //Act
            var times = new MultiplicationOperation(lhs, rhs);

            //Assert
            var expected = lhs.Value * rhs.Value;
            Assert.AreEqual(expected, times.Value);
        }
        ...
        public void Should_set_expression_to_sin_of_baseOperator_expression()
        {
            ...
            //Assert
            var expected = "(left) x right";
            Assert.AreEqual(expected, times.Expression);
        }
    }
}
